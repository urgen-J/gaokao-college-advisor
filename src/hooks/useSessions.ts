...
// 本地缓存 key 按登录账号隔离，换账号不会串数据
const sessionModelsKey = () => scopedKey('sessionModels');
...
export function useSessions() {
  ...
  const [sessionModels, setSessionModels] = useState<Record<string, string>>(() => {
    try {
      const saved = localStorage.getItem(sessionModelsKey());
  ...
  const updateSessionModel = useCallback((sessionId: string, modelId: string) => {
    setSessionModels(prev => {
      ...
      localStorage.setItem(sessionModelsKey(), JSON.stringify(updated));
      return updated;
    });
    setSessions(prev => prev.map(s => 
