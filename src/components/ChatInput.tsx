...
import { Select, Tooltip, Checkbox } from 'tdesign-react';
    ...
    >
      <div className="max-w-3xl mx-auto">
        <ChatSender
          ref={chatSenderRef}
          value={inputValue}
          placeholder="例如：我是河南理科考生，620分，位次1.2万，想学计算机，推荐哪些学校？"
        ...
        >
          ...
          <div slot="footer-prefix" className="flex items-center gap-2">
            ...
            {/* 联网搜索勾选框 */}
            <Tooltip content={webSearch ? "已开启联网搜索" : "未开启联网搜索（仅使用本地与知识库）"} placement="top">
              <Checkbox
                checked={webSearch}
                onChange={(value) => onWebSearchChange(Boolean(value))}
              >
                联网搜索
              </Checkbox>
            </Tooltip>
            {webSearch && !qwenConfigured && (
              <span style={{ fontSize: 12, color: '#e34d59', marginLeft: 4, whiteSpace: 'nowrap' }}>
                未配置联网搜索所需的密钥，本次仅使用本地与知识库
              </span>
            )}
          </div>
        </ChatSender>
